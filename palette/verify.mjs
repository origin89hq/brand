...
import { GROUNDS, HERITAGE, QUALITY, TOKENS } from "./src/palette.mjs";
...
for (const token of TOKENS) {
  ...
  for (const theme of ["dark", "light"]) {
    ...
    if (token.check === "legend") {
      ...
      if (!legend)
        throw new Error(`${token.name} names a legend that does not exist: ${token.legend}`);
...
if (failures.length) {
  ...
  for (const f of failures) {
    console.error(
      `    ${f.token} (${f.theme}) is ${f.ratio.toFixed(2)}:1 against ${f.against}, needs ${f.floor}`,
    );
  }
  console.error();
  process.exit(1);
