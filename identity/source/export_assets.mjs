...
const require = createRequire(process.env.O89_NODE_MODULES ? process.env.O89_NODE_MODULES + '/' : import.meta.url);
...
for(const file of await fs.readdir(path.join(root,'logos'))){
  ...
  await sharp(input,{density:300}).resize({width:2000}).png().toFile(path.join(root,'logos/png',file.replace('.svg','.png')));
}
for(const size of [16,24,32,48,64,128,180,192,256,512,1024]){
  await sharp(path.join(root,'icons/offgrid-flat-master.svg'),{density:300}).resize(size,size).png().toFile(path.join(root,`icons/offgrid-flat-${size}.png`));
}
const cols=['blue','black','white'];
let body='<svg xmlns="http://www.w3.org/2000/svg" width="1800" height="900"><rect width="1800" height="900" fill="#f4f3ef"/>';
let y=40;
...
for(const size of [16,32,48])await sharp(Buffer.from(favicon),{density:400}).resize(size,size).png().toFile(path.join(root,`icons/favicon-${size}.png`));
try{
 for(const size of [32,48,64,128,180,192,256,512,1024])await sharp(path.join(root,'renders/icon.png')).resize(size,size).removeAlpha().png().toFile(path.join(root,`icons/offgrid-3d-${size}.png`));
}catch(e){if(e.code!=='ENOENT'&&!e.message.includes('missing'))throw e;}
const logowhite=await fs.readFile(path.join(root,'logos/origin89-horizontal-white.svg'),'utf8');
const controller=await fs.readFile(path.join(root,'logos/origin89-controller-white.svg'),'utf8');
function nested(svg,x,y,w,h){return svg.replace('<svg ',`<svg x="${x}" y="${y}" width="${w}" height="${h}" `);}
...
await sharp(Buffer.from(label),{density:150}).resize(1600).png().toFile(path.join(root,'applications/controller-label-study.png'));
