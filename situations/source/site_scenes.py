"""Cottage, telecom shelter and mine utility site models, shared by the render scripts.

Self-contained: each scene is modelled here from primitives in an empty file, with
no character or hardware input. The equipment is generic and unbranded, in the same
world as the website's Controller renders: dark satin materials, a soft key, a blue
rim, one green status light per site and blue signal paths on the ground.

A scene builder takes the palette and a `ground` callable, so a caller chooses what
the equipment stands on: `plinth` for the miniature dioramas, a wide field for the
site views. Builders append their signal paths to `SIGNALS` as (name, points) in
world space; a caller clears it before building.

Renders live in build_site_miniatures.py (orthographic transparent dioramas) and
build_site_views.py (wide perspective views over a field).
"""
import math
