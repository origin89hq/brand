...
const stage = join(here, "dist", "origin89-brand-kit"), dist = join(here, "dist");
rmSync(stage, { recursive: true, force: true });
mkdirSync(stage, { recursive: true });
for (const f of readdirSync(join(here, "identity/logos")).filter((f) => /\.(svg|png)$/.test(f))) cpSync(join(here, "identity/logos", f), join(stage, "logos", f));
for (const f of readdirSync(join(here, "identity/icons")).filter((f) => /\.(svg|png|ico)$/.test(f))) cpSync(join(here, "identity/icons", f), join(stage, "icons", f));
for (const f of readdirSync(join(here, "identity/fonts")).filter((f) => /\.(ttf|txt)$/.test(f))) cpSync(join(here, "identity/fonts", f), join(stage, "fonts", f));
cpSync(join(here, "identity/guide/output/pdf/origin89-design-guide.pdf"), join(stage, "origin89-design-guide.pdf"));
