...
| `situations/` | Editable Buddy scenes and their build scripts: Reserve at a site, and the equipment scout holding binoculars for the Data page. Also the script for the website's site miniatures. |
| `source/` | `buddy.py`, the driver for rebuilding the character, its presentation renders and avatars. Needs Blender. |
| `palette/` | The 21 colour tokens, their light and dark values, and why each one exists. The only place a colour is written. `verify.mjs` measures every value against the contrast floor it claims. |
